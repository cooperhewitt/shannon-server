...
GUNICORN_OPTS='-b 127.0.0.1:6521'
...
case $1 in
	...
	cd $SHANNON_ROOT
	exec sudo -u www-data $GUNICORN $GUNICORN_OPTS shannon:application
        ;;
    start)
        echo "Starting shannon server"

	cd $SHANNON_ROOT
	PID=`exec sudo -u www-data $GUNICORN $GUNICORN_OPTS shannon:application > /dev/null 2>&1 & echo $!`
